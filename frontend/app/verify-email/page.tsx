...
function VerifyEmailContent() {
  ...
  useEffect(() => {
    // Show loading briefly, then check token
  ...
  const verifyEmail = async (token: string) => {
    try {
      ...
      if (data.status === "verified") {
        ...
        toast.error(data.msg || "Ошибка подтверждения email");
      }
    } catch (err) {
      ...
      <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-background px-4 py-12">
        <div className="w-full max-w-md">
          <div className="border border-border bg-card p-8 text-center">
            ...
            <h1 className="mb-2 text-2xl font-bold">Email подтвержден!</h1>
            <p className="mb-6 text-sm text-muted-foreground">
              Ваш email успешно подтвержден. Сейчас вы будете перенаправлены...
            </p>
