...
| Pricing-flow unit tests | Effective Paid-expiry state, remaining-time formatting, plan-change API requests, safe internal login redirects, and registration/verification redirect persistence. | `npm test -- --coverage`, `npm run check` in `frontend/` | Passing locally on 2026-07-16; protected-branch CI evidence is pending. | Frontend test sources in `frontend/lib/*.test.ts`; local result is not release or CI evidence. |
